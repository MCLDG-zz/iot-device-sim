...
function getColoredText(a, b) {
    return a < 50 ? b.blue + " (COLD!)".gray : a < 65 ? b.cyan + " (BRISK)".gray : a <
        80 ? b.yellow + " (WARM)".gray : b.red + " (HOT!!)".gray
}
...
function run(a) {
    ...
    device.publish(topic, b), console.log("Published ".gray + getCol oredText(temp, b) + "to AWS IoT.".gray)
}
...
console.log("Connecting to AWS IoT...".blue),
    ...
    device.on("connect", function() {
        ...
        device.register(config.app.thingName), setInterval(run, PUBLISH_INTERVAL), setInterval(reportState, STATUS_INTERVAL)
    }),
    ...
    device.on("delta", function(a, b) {
        try {
            console.log("Reported state different from remote state.".gray), b.state.airConditioningIsOn && b.state.airConditioningIsOn != airConditioningIsOn ? (airConditioningIsOn = b.state.airConditioningIsOn, a irConditioningIsOn ? console.log("Air conditioning is now on.".green) : console.log("Air conditioning is now off.".red)) : console.log("No new desired state found.".gray)
        } catch (a) {
